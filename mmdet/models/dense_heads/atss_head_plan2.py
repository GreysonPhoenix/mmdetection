...
class ATSSHead(AnchorHead):
    ...
    def forward_single(self, x, scale, labels, bbox_target, anchors, stride):
        ...
        cls_aware = torch.autograd.grad(out, inputs, grad_outputs=prob_outputs.clone(), retain_graph=True)[0]
        cls_mask = (cls_aware / cls_aware.max(dim=-1, keepdim=True))
        cls_aware[cls_mask > 0.5] = 0
        confused = torch.autograd.grad(out, inputs, grad_outputs=confuse_outputs.clone(), retain_graph=True)[0]

        w = 1.5 * cls_aware.sigmoid() - 1.5 * confused.sigmoid()
        reg_feat = (0.5 + w) * x
        cls_feat = (0.5 + w) * x
        for cls_conv in self.cls_convs:
